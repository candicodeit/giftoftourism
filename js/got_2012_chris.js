...
$(document).ready(function () {
	...
	$('a.learn').unbind('click').click(function(){
		...
		console.log(array_deals[dealNum].title);
		
		console.log(deals_open + ' location' + location.hash);
...
function checkHash(state){
	...
	if(state=="init"){
		...
		if(location.hash!="#/"&&location.hash!=""&&location.hash!="#/deals/"){
			...
			toggleDeals();
		/*} else if(location.hash=="#/deals/"&&deals_open&&dealNum!=''){	
			console.log('WORK')*/
		} else {
			section = "";
...
function buildDeal(index){
	console.log('run plus index' + index);
	console.log(array_deals[index]);
