...
$(document).ready(function () {
	...
	$('.prev-deals, .next-deals').unbind('click').live('click',function(){
		lasthash = $(this).attr('href'); 
		dealNum = parseInt($(this).attr('href').substr(-2).replace('/', ''));
	...
	$('.enter_now').unbind('click').live('click',function(){
		...
		$('.entry').clone().appendTo( target ).show()
				.find('#contest_type').attr('value', array_deals[dealNum].title).end()
				.find('#email_recipient').attr('value', array_deals[dealNum].recipient);	
	});
	...
	$("#deal_overlay #entry_submit").bind("click", function(){
		handleEntrySubmit();
	});
});
		...
		if(location.hash!="#/"&&location.hash!=""){
			...
			if(tmpsection=="lifestyle"){
				...
				if(dealType == "contest"){
					...
					$('.entry').clone().appendTo( target ).show()
						.find('#contest_type').attr('value', array_deals[deephash].title).end()
						.find('#email_recipient').attr('value', array_deals[deephash].recipient);	
				} else {
					changeContent("nav_deals");
					deeplink = true;
...
var array_deals = [
	...
	{		// Deal 1
			deal_type:"coupon", 
			title:"THE VIRGINIA AQUARIUM &amp;<br /> MARINE SCIENCE CENTER", 
...
function handleEntrySubmit(){
	if($("#deal_overlay #txt_fname").val()!=""){
		$.post("entry.asp", {
			contest_type:$("#deal_overlay #contest_type").val(), 
			email_recipient:$("#deal_overlay #email_recipient").val(),
			txt_fname:$("#deal_overlay #txt_fname").val(), 
			txt_lname:$("#deal_overlay #txt_lname").val(),
			txt_address:$("#deal_overlay #txt_address").val(), 
			txt_city:$("#deal_overlay #txt_city").val(),
			select_state:$("#deal_overlay #select_state").val(), 
			txt_zip_code:$("#deal_overlay #txt_zip_code").val(),
			txt_phone: $("#deal_overlay #txt_phone").val(), 
			txt_email_addr: $("#deal_overlay #txt_email_addr").val(), 
			txt_email_confirm:$("#deal_overlay #txt_email_confirm").val(), 
			check_age: $("#deal_overlay #check_age").attr("checked"), 
			check_terms: $("#deal_overlay #check_terms").attr("checked")
			}, function(data) {
			if(data=="sent"){
				var target = $('#deal_overlay');
				target.empty();
