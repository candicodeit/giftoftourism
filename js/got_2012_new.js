...
$(document).ready(function () {
	...
	$('.enter_now').unbind('click').live('click',function(){
		...
		$('.entry').clone().appendTo( target ).show()
				.find('#contest_type').attr('value', array_deals[dealNum].title);	
	});
...
function handleEntrySubmit(){
	if($("#deal_overlay #txt_fname").val()!=""&&$("#deal_overlay #txt_email_addr").val()&&$("#deal_overlay #check_age").getAttribute("checked")=="checked"&&$("#deal_overlay #check_terms").getAttribute("checked")=="checked"){
		$.post("entry.asp", {
			...
			txt_email_confirm:$("#txt_email_confirm").val(), 
